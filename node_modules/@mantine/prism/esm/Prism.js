...
import { useMantineDefaultProps, useMantineTheme, ScrollArea, Box, Tooltip, ActionIcon, Tabs } from '@mantine/core';
import { useClipboard } from '@mantine/hooks';
import { CopyIcon } from './CopyIcon.js';
import { getPrismTheme } from './prism-theme.js';
import useStyles from './Prism.styles.js';
import useTabsStyles from './PrismTabs.styles.js';
...
const Prism = forwardRef((props, ref) => {
  const _a = useMantineDefaultProps("Prism", prismDefaultProps, props), {
    className,
    children,
    language,
    noCopy,
  ...
  const { classes, cx } = useStyles({ colorScheme: colorScheme || theme.colorScheme, native: ScrollAreaComponent !== ScrollArea }, { classNames, styles, name: "Prism" });
  return /* @__PURE__ */ React.createElement(Box, __spreadValues({
    className: cx(classes.root, className),
    ref
  }, others), !noCopy && /* @__PURE__ */ React.createElement(Tooltip, {
...
const PrismTabs = forwardRef((props, ref) => {
  const _a = useMantineDefaultProps("PrismTabs", {}, props), { children, classNames, styles } = _a, others = __objRest(_a, ["children", "classNames", "styles"]);
  const { classes, cx } = useTabsStyles(null, { name: "PrismTabs", classNames, styles });
  const tabs = Children.toArray(children).filter((child) => child.type === PrismTab).map((child, index) => {
    const _a2 = child.props, { label, icon } = _a2, prismProps = __objRest(_a2, ["label", "icon"]);
    return /* @__PURE__ */ React.createElement(Tabs.Tab, {
      label,
      icon,
      key: index
    }, /* @__PURE__ */ React.createElement(Prism, __spreadProps(__spreadValues({}, prismProps), {
      styles,
...
export { Prism, PrismTab, PrismTabs };
